    ...
    $.extend($, {
        ...
        initLoginBox: function (containerID, confirmButton, handler) {
            ...
            container.find("input").each(function () {
                switch (this.name) {
                    ...
                    case "password":
                        ...
                        break;
                    case "vercode":
                        $(this).attr("data-prompt", "['请输入登录验证码！', '登录验证码长度不正确！']").attr("data-regex", "[/^\\S+$/,/^\\S{4}$/]");
                        break;
                }
            }).keydown(function (e) {
                if (e.keyCode === 13)
            ...
            container.find(confirmButton).click(function () {
                ...
                if (data) {
                    if ($.queryString("ip"))
                        data.ip = $.queryString("ip");
                    $.ajax("/doLogin", data, function (response) {
